...
def create_activity():
    ...
    cur.execute("insert into appl1011(uuid, courseid, credit, memberid, begintime, objectstate) values(%s,%s,%s,%s,%s,%s)",
             (uuid.uuid1(),data['course_uuid'], data['course_credit'], data['member_uuid'], data['timeValue'],data['objectvalue']))
    conn.commit()
    return "1"
...
def read_one_activity(mid):
    ...
    for i,row in enumerate(rows):
        ...
        print("----------------")
        # for i in courses:
        #     cour = {'code':str(i[4]),'name':str(i[6]),'objectstate':int(i[18])}
        #     print(int(i[18]))
        #     l.append(cour)
    print(l)
    return jsonify(l)
...
def read_member():
    ...
    for row in rows:
        # print(row)
        dic= {'id': str(row[0]),'employeeid': str(row[14]),'firstname':str(row[5]),'uuid':str(row[1]),'objectstate':str(row[27])}
        l.append(dic)
    return jsonify(l)
...
def read_course():
    ...
    for row in rows:
        # print(row)
        dic= {'id': str(row[0]),'code': str(row[3]),'name':str(row[5]),'uuid':str(row[22]),'credit':str(row[9]),'objectstate':str(row[16])}
        l.append(dic)
    return jsonify(l)
...
# @app.route('/api/u/activity/<string:name>/<memberId>', methods=['DELETE'])
# def delete_act(name,memberId):
#     cur = conn.cursor()
...
#     member_id = "'"+memberId+"'"
#     cur.execute("DELETE from appl1011 where courseid={} and memberid={}".format(course_id,member_id))
#     conn.commit() 
#     return "1"
...
# def read_avtivities():
#     cur = conn.cursor()
#     cur.execute("select APPL1011.uuid,APPL1011.category,APPL1011.memberid,APPL1011.begintime,APPL1011.endtime,APPL1011.workday,APPL1011.workpoint,MAST0501.employeeid,MAST0501.firstname,MAST0501.title,APPL1011.courseid,MAST1014.id,MAST1014.name,MAST1014.credit from APPL1011  join MAST0501 ON(APPL1011.memberid=MAST0501.uuid) join MAST1014 ON(APPL1011.courseid=MAST1014.uuid)")
#     rows = cur.fetchall()
#     l = []
#     for row in rows:
#         print(row)
#         dic= {'uuid': str(row[2]),'employeeid': str(row[7]),'name':str(row[8]),'title':str(row[9]),'courseuuid':str(row[10]),'courseid':str(row[11]),'coursename':str(row[12]),'credit':str(row[13]),'begintime':str(row[3]),'endtime':str(row[4]),'workday':str(row[5]),'workpoint':str(row[6])}
#         l.append(dic)
#     print(l)
#     return jsonify(l)
...
def update_one_activity(id):
    ...
    cur.execute("UPDATE APPL1011 SET uuid = '{}', memberid = '{}', courseid = '{}', begintime = '{}', endtime = '{}', workday = '{}' ,workpoint = '{}' WHERE id = {}"
        .format(data['uuid'], data['memberid'],data['courseid'],data['begintime'], data['endtime'], data['workday'], data['workpoint'], id))
...
@app.route('/api/admin/tasks/new',methods=['POST'])
def create_task():
    cur = conn.cursor()
    data = request.get_json()
...
@app.route('/api/admin/tasks/<string:id>',methods=['GET'])
...
@app.route('/api/admin/tasks/<string:title>',methods=['GET'])
...
@app.route('/api/admin/tasks/v/<string:id>', methods=['PUT'])
...
@app.route('/api/admin/tasks/e/<string:id>', methods=['DELETE'])
...
def read_avtivities():
    ...
    for row in rows:
        print(row)
        dic= {'uuid': str(row[2]),'employeeid': str(row[7]),'name':str(row[8]),'title':str(row[9]),'courseuuid':str(row[10]),'courseid':str(row[11]),'coursename':str(row[12]),'credit':str(row[13]),'objectstate':str(row[14]),'begintime':str(row[3]),'endtime':str(row[4]),'workday':str(row[5]),'workpoint':str(row[6])}
        if (str(row[14]) == "1"):
            dic['objectstate'] = "申请中"
        else:
            dic['objectstate'] = "已分配"
...
def read_activity_by_membername(name):
    ...
    memberid = "'" + id + "'"
    cur.execute(
            "select APPL1011.uuid,APPL1011.category,APPL1011.memberid,APPL1011.begintime,APPL1011.endtime,APPL1011.workday,APPL1011.workpoint,MAST0501.employeeid,MAST0501.firstname,MAST0501.title,APPL1011.courseid,MAST1014.id,MAST1014.name,MAST1014.credit,APPL1011.objectstate from APPL1011  join MAST0501 ON(APPL1011.memberid=MAST0501.uuid) join MAST1014 ON(APPL1011.courseid=MAST1014.uuid) where memberid={}".format(memberid))
    rows = cur.fetchall()
    l = []
    for row in rows:
        print(row)
...
def read_node():
    ...
    for row in rows:
        print(row)
        dic= {'uuid': str(row[0]),'originname': str(row[1]),'curname':str(row[2]),'upddate':str(row[3]),'crtdate':str(row[4])}
        l.append(dic)
        print(dic.values())
    print(l)
    conn.commit()
